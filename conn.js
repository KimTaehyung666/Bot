...
const { hentaivid } = require("./function/scrape_hentai")
...
"fileName": `BotKntl-MD ${ngazap(prefix)}`,
...
*_Seputar botkntl & transaksi_*

${strip}Telpon/Spam blokir 🚫${strip}

_Admin 1 : 0813-1349-0078_
...
break
case "faktaunik":
fetchJson(`https://saipulanuar.ga/api/fakta`).then(res =>{
	reply(`${res.result}`)
})
break
case "lirik":
if (!q) return reply('Contoh:\n.lirik one ok rock - we are')
fetchJson(`https://api.lolhuman.xyz/api/lirik?apikey=${setting.api_lolkey}&query=${q}`).then(res =>{
	reply(`${res.result}`)
})
break
...
case 'ouo':
if (!q) return reply('Contoh:\n#ouo (link)')
fetchJson(`https://api.lolhuman.xyz/api/ouo?apikey=${setting.api_lolkey}&url=${q}`).then((res) => {
	let hasil = `Selamat link anda Terbypass
...
case 'mirrored':
if (!q) return reply('Contoh:\n#mirrorred (link)')
fetchJson(`https://api.lolhuman.xyz/api/mirrorcreator?apikey=${setting.api_lolkey}&url=${q}`).then((res) => {
	let hasil = ` Selamat link anda terbypass
	
Zippyshare ${res.result.zippyshare}
Gofile ${res.result.gofileio}
Usercloud ${res.result.userscloud}
Racaty ${res.result.racaty}
Drive ${res.result.googledrive}
Dropapk ${res.result.dropapk}
Videobinco ${res.result.videobinco}`
	reply(hasil)
})
break
...
case 'spotify':
if (!q) return reply('Contoh:\n#spotify linkdl')
fetchJson(`https://api.lolhuman.xyz/api/spotify?apikey=${setting.api_lolkey}&url=${q}`).then((res) => {
	let spo = `
Title ${res.result.title}
Penyanyi ${res.result.artists}
Durasi ${res.result.duration}
Popularity ${res.result.popularity}

Link dl ${res.result.link}`
	reply(spo)
})
break
case "host":
if (!q) return reply("Masukan hostname")
axios.get(`https://api.hackertarget.com/hostsearch/?q=${q}`).then((res) => {
	reply(`${res.data}`)
})
break
////
...
.then(res => {
reply(`Author : BotKntl

Judul : ${res.result.description}

Video sedang dikirim...`)
conn.sendMessage(from,{video:{url:res.result.video}, caption:'No Watermark!'}, {quotes:msg})
}).catch((err) => {
...
if (!q) return reply("contoh :\n#tiktok https://vt.tiktok.com/ZSRG695C8")
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/download/tiktok4?url=${q}`)
.then((res) => {
	reply(`Author : BotKntl

Judul : ${res.result.author}

Video sedanf dikirim.....`)
conn.sendMessage(from, {video:{url:res.result.videoHD}, caption:"No Watermark!!"}, {quoted:msg})
}).catch((err) => {
reply('Terjadi Kesalahan!!\nUrl tidak valid')
})
}
...
//// instagram down
case 'igstalk':
if (!q) return reply('Masukan uname ig')
fetchJson(`https://saipulanuar.ga/api/stalk/ig?username=${q}`)
.then((res) => {
	reply(`Username : ${res.result.username}
...
// darkjokes new
case "darkjokes2":{
let iuh = `https://saipulanuar.ga/api/darkjoke`
conn.sendMessage(from, { image:{url:iuh}, caption:"Nih DarkJokes",mention:[sender]},{quoted: msg})
}
break

// kisah nabi
case 'kisahnabi':
if (!q) return reply('.kisahnabi muhammad')
fetchJson(`https://saipulanuar.ga/api/muslim/kisahnabi?nabi=${q}`).then((res) => {
	reply(`${res.result.name}
...
// stalkgithub
case "stalkgithub2":{
if (!q) return reply(".stalkgithub2 (username)")
fetchJson(`https://api.github.com/users/${q}`).then((res ) => {
reply(`login : ${res.login}
id : ${res.id}
...
// quotenime
case 'quoteanime':
fetchJson(`https://api.lolhuman.xyz/api/random/quotesnime?apikey=${setting.api_lolkey}`).then((res) => {
	reply(`${res.result.quote}
...
case 'brainly':
if (!q) return reply('Contoh:\n#brainly apa itu javascript')
fetchJson(`https://api.xteam.xyz/brainly?soal=${q}&APIKEY=6fa933d23cc835ed`).then((res) => {
	reply(`${res.jawaban}`)
})
break
case 'wikimedia':{
...
var nyz1 = await getBuffer(`https://saipulanuar.ga/api/maker/ttp2?text=${q}`)
...
.then(z=>{
let text_playmp3 =`*YOUTUBE PLAYMP3*
...
conn.sendMessage(sender, { video: { url: cejd.video_1 }, 
...
⭔ Source : ${cejd.link}
⭔ Media Url : ${cejd.video_1}` }, { quoted: msg })
}
break
...
var opt = { packname: 'Bot Kntl', author: 'VaL' }
...
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
reply(mess.wait)
let ber = `https://api.lolhuman.xyz/api/random/art?apikey=${setting.api_lolkey}`
conn.sendMessage(from, { image:{url:ber}, caption:"Nih fanart",mention:[sender]},{quoted: msg})
}
break
// meme indo
case 'meme':
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
reply(mess.wait)
let yoi = `https://saipulanuar.ga/api/memeh`
conn.sendMessage(from, { image: { url:yoi}, caption:`Nih ${command}`},{quoted:msg})
break
// meme world
case "memeworld":{
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
reply(mess.wait)
let ih = `https://saipulanuar.ga/api/random/meme`
conn.sendMessage(from, { image: { url:ih}, caption:`Nih ${command}`},{quoted:msg})
}
break
